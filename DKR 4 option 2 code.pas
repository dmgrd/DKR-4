...
begin
  var fo: real;
  fo := (1*power(x,3)+(-1)*power(x,2)+(3)*x+(5));
  fun := fo;
end;
...
begin
  var fo: real;
  fo := 1/2*power(x,4)-1/3*power(x,3)+5/2*power(x,2)+3*x;
  fun1 := fo;
...
begin      
  ...
  Writeln('Вычисление площади фигуры, ограниченной кривой 1*x^3+(-1)*x^2+(3)*x+(5) и осью Ох (в положительной части по оси Оу)');
